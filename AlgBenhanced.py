...
my_first_name = "Oluwatobi"
my_last_name = "Afolabi"
...
threshold = 10  # Define a threshold for fitness (distance)
elite_size = 6  # Number of elite individuals to preserve
pop_size=300
mutation_rate=0.05 
max_it=2000 
max_time = 55
...
def calculate_distance(route, distances):
    ...
    total_length += distances[route[-1]][route[0]]  # Return to the starting city
    return total_length
...
def crossover(parent1, parent2):
    ...
    # Fix duplicates in the child using the order of occurrence in the other parent
...
def genetic_algorithm_with_elitism(distances, pop_size, mutation_rate, max_it, elite_size, threshold, max_time):
    ...
    iteration = 0
    while iteration < max_it and time.time() - start_time < max_time:
        new_population = []
        
        # Evaluate fitness for each individual
        fitness = [(individual, calculate_distance(individual, distances)) for individual in population]
        fitness.sort(key=lambda x: x[1])  # Sort by fitness
        
        # Preserve elites
        elites = [individual for individual, _ in fitness[:elite_size]]
        new_population.extend(elites)
        
        # Update best solution
        if fitness[0][1] < best_fitness:
            best_tour = fitness[0][0]
    ...
    tour_length = calculate_distance(best_tour, distances)
    return best_tour, tour_length

tour, tour_length = genetic_algorithm_with_elitism(dist_matrix, pop_size, mutation_rate, max_it, elite_size, threshold, max_time)
