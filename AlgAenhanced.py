...
def hill_climbing_tsp(distance_matrix, max_iterations=1000, max_restarts=10):
    ...
    def generate_successor(current_route):
        i, j = random.sample(range(len(current_route)), 2)
        successor = current_route[:]
    ...
    def local_search(initial_route):
        ...
        for _ in range(max_iterations):
            successor = generate_successor(current_route)
            successor_cost = calculate_tour_cost(successor)

            if successor_cost < best_cost:
                best_route = successor[:]
                best_cost = successor_cost

            if successor_cost < current_cost:
                current_route = successor[:]
    ...
    for _ in range(max_restarts):
        initial_route = random.sample(range(len(distance_matrix)), len(distance_matrix))
        tour, length = local_search(initial_route)
...
tour, tour_length = hill_climbing_tsp(dist_matrix)
