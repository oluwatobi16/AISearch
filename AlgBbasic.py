...
def genetic_algorithm(distances, pop_size, mutation_rate, max_it, threshold, max_time):
    ...
    while iteration < max_it and time.time() - start_time < max_time:
        ...
        population = [individual for individual, _ in sorted(fitness, key=lambda x: x[1])]
        # Check if the fittest individual is better than the current best
...
tour, tour_length = genetic_algorithm(dist_matrix, pop_size, mutation_rate, max_it, threshold, max_time)
